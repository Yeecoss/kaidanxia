...
<template>
	<view class="interlayer">
		<block v-for="(item, index) in nodes" v-bind:key="index">
			...
			<rich-text style="word-wrap: break-word;word-break: break-all;" v-else-if="item.decode" class="_entity" :nodes="[item]"></rich-text>
			<!--#endif-->
			...
			<view v-else-if="item.name=='video'">
				...
				<view v-if="(!loadVideo||item.lazyLoad)&&(!controls[item.attrs.id]||!controls[item.attrs.id].play)" :id="item.attrs.id"
				 class="_video1" :style="item.attrs.style" @tap="_loadVideo">
				 <image mode="aspectFill" style="width: 100%;height: 100%;" :src="(controls[item.attrs.id] ? item.attrs.source[controls[item.attrs.id].index] : item.attrs.src) + '?vframe/jpg/offset/0|imageView2/1/w/375/h/375'"></image>
			...
			<!--富文本-->
			<!--#ifdef MP-WEIXIN || MP-QQ || MP-ALIPAY || APP-PLUS-->
			<rich-text style="word-wrap: break-word;word-break: break-all;" v-else-if="handler.useRichText(item)" :id="item.attrs.id" :class="'_p __'+item.name" :nodes="[item]" />
			<!--#endif-->
			<!--#ifdef MP-BAIDU || MP-TOUTIAO-->
			<rich-text style="word-wrap: break-word;word-break: break-all;" v-else-if="!(item.c||item.continue)" :id="item.attrs.id" :class="_p" :style="item.attrs.contain"
			 :nodes="[item]" />
			<!--#endif-->
			<!--#ifdef MP-ALIPAY-->
			...
			<!--#endif-->
			<!--#ifdef APP-PLUS-->
			<div class="videoBox dis-flex flex-y-center" @tap="closeVideo" :id="item.attrs.id" v-if="showVideo && !((!loadVideo||item.lazyLoad)&&(!controls[item.attrs.id]||!controls[item.attrs.id].play))">
				<video style="width: 100%;" :autoplay="item.attrs.autoplay||(controls[item.attrs.id]&&controls[item.attrs.id].play)"
					 :controls="item.attrs.controls" :loop="item.attrs.loop" :muted="item.attrs.muted" :poster="item.attrs.poster" :src="controls[item.attrs.id] ? item.attrs.source[controls[item.attrs.id].index] : item.attrs.src"
					 :unit-id="item.attrs['unit-id']" :data-source="item.attrs.source" data-from="video" @play="play" @error="error" />
			</div>
			<!--#endif-->
