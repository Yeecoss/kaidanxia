<template>
	<view class="tui-tabs-view" id="tui-tabs" :class="[isFixed?'tui-tabs-fixed':'tui-tabs-relative',unlined?'tui-unlined':'']" :style="{height:height+'rpx',padding:`0 ${padding}rpx`,background:backgroundColor, 'align-items': isTitle ? 'left' : 'center'}"><!-- top:isFixed?top+'px':'auto', -->
	...
	export default {
		...
		methods: {
			checkCor: function() {
				...
				let width = 0
				if (this.width) {
					width = this.width * this.winWidth / 750 - padding * 2
				} else {
					width = this.winWidth - padding * 2
				}
