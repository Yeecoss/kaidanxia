<template>
	<view class="minishop-list">
			<view class="mbg"></view>
			<checkbox-group class="goods-list" @change.stop="choose">
				<view class="goods-list-wrap">
					<view class="goods-item" v-for="(dataItem, index) in listData" :key="dataItem.article_id" v-if="(index + 1) % 2 != 0" @tap.stop="toDetail(dataItem.article_id)">
						<!-- 商品图片 -->
						<view class="goods-item_pic check-wrap">
							<image :src="dataItem.image.file_path" mode="widthFix"></image>
							<checkbox @tap.stop v-show="ifManage" class="checkbox" :value="dataItem.article_id+''" :checked="dataItem.checked" />
						</view>
					
						<view class="detail">
							<!-- 商品名称 -->
							<view class="goods-name twolist-hidden col-3 f-26">
							...
							<view class="desc_footer dis-flex flex-y-center flex-x-between">
								...
								<view class="f-24 col-9 read-num"><text class="iconfont2 icon-eye f-24"></text>{{dataItem.show_views}}</view>
							</view>
						</view>
					</view>
				</view>
				<view class="goods-list-wrap">
					<view class="goods-item" v-for="(dataItem, index) in listData" :key="dataItem.article_id" v-if="(index + 1) % 2 == 0" @tap="toDetail(dataItem.article_id)">
						<!-- 商品图片 -->
						<view class="goods-item_pic check-wrap">
							<image :src="dataItem.image.file_path" mode="widthFix"></image>
							<checkbox @tap.stop v-show="ifManage" class="checkbox" :value="dataItem.article_id+''" :checked="dataItem.checked" />
			...
			<view class="fot-operation dis-flex flex-y-center flex-x-between" v-if="ifManage">
				...
				<view class="fot-opera-r dis-flex flex-y-center">
					...
					<view class="fot-del f-24 col-6 p-l-10 p-r-10" @tap.stop="delData"><text class="iconfont2 icon-shanchu"></text>删除</view>
