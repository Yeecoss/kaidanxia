				...
				<view class="goods-list-wrap">
					<view class="goods-item" v-for="(dataItem, index) in listData" :key="dataItem.article_id" @tap.stop="toDetail(dataItem.article_id)">
						...
						<view class="goods-item-top">
							<view v-if="type=='marketing'" class="goods-decr flex-box col-2a f-30 m-r-20">
								<text class="three-hidden">{{ dataItem.article_title }}</text>
							</view>
							<view class="goods-item_pic check-wrap">
								...
								<!-- 商品名称 -->
								<view v-if="type=='wantbuy'" class="goods-name onelist-hidden col-f f-30">
									<text>{{ dataItem.article_title }}</text>
								</view>
							</view>
						</view>
					
						<view class="detail dis-flex flex-y-center">
							<view class="detail-left flex-box oh dis-flex flex-y-center">
								...
								<view class="user-info m-l-20 flex-box">
									...
									<template v-if="type=='wantbuy'">
										<view class="has-num f-24 m-t-10" v-if="dataItem.goods_relation_count>0"><text class="iconfont2 icon-commodity f-22"></text>内容含{{dataItem.goods_relation_count}}个商品</view>
									</template>
									<view class="f-24 theme4_gray m-t-10" v-else>{{dataItem.create_time.split(' ')[0]}}</view>
								</view>
							</view>
							<view class="separator oh"></view>
							<view class="desc_footer t-c oh">
								<view class="f-32 col-2a read-num">{{dataItem.show_views}}</view>
								<view class="f-24 theme4_gray m-t-10">多少人看过</view>
							</view>
						</view>
					</view>
				</view>
