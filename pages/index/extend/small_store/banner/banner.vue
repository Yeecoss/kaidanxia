...
<view class="diy-banner" :class="styleClass">
  <swiper :autoplay="autoplay" :class="'banner-box swiper-box dot-' + itemStyle.btnShape" :duration="duration" circular :indicator-dots="indicatorDots" :interval="interval" :indicator-color="itemStyle.btnColor" indicator-active-color="#000" @change="bindChangeFun" :data-item-key="itemIndex" style="height: 300rpx">
    <swiper-item class="swiper-item" :for-item="banner" v-for="(banner, index) in dataList" :key="index">
      <image lazy-load mode="aspectFit" @tap.stop="navigationTo(banner)" class="slide-image" @load="imagesHeightFun" :src="banner.imgUrl" :data-id="index" :data-item-key="itemIndex"></image>
    </swiper-item>
  </swiper>
  <view class="dots">
    <view class="dots-item" :class="imgCurrent == index ? 'on' : ''" v-for="(item, index) in dataList" :key="index">
    </view>
  </view>
  <!-- 顶部置灰 -->
