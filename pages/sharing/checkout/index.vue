...
<view class="container p-bottom" v-if="goods_list.length">
  ...
  <view class="content">
    <!-- 快递配送：配送地址 -->
    <view v-if="curDelivery == DeliveryTypeEnum.EXPRESS.value" @tap.stop="onSelectAddress" class="flow-delivery boxShadow">
    ...
    <block v-if="curDelivery == DeliveryTypeEnum.EXTRACT.value">
      <view @tap.stop="onSelectExtractPoint" class="flow-delivery boxShadow dis-flex flex-y-center">
        <view class="address_border"></view>
        <view class="flow-delivery__detail dis-flex flex-y-center">
          <view class="detail-location dis-flex">
            <text class="iconfont icon-dingwei"></text>
    ...
    <!-- 商品列表 -->
    <view class="m-top20 boxShadow checkout_list_box">
            ...
            <view class="flow-shopList dis-flex" :data-index="index">
              ...
              <view class="flow-list-right flex-box">
                <view class="goods_name">
                  <!-- 商品名称 -->
                  <text class="f-24 col-3 twolist-hidden">{{ item.goods_name }}</text>
                  <!-- 商品sku信息 -->
      ...
      <view class="flow-num-box b-f padding-box">
        <text class="total">共{{order_total_num}}件商品</text>
        <text class="flow-money">
          <text style="color: #999990;">合计：</text>
    ...
    <!-- 商品金额 -->
    <view class="flow-all-money b-f m-top20 boxShadow">
    ...
    <!-- 支付方式 -->
    <view class="boxShadow pay-method flow-all-money b-f m-top20">
      <!-- <view class="flow-all-list dis-flex">
    ...
    <!-- 买家留言 -->
    <view class="boxShadow flow-all-money b-f m-top20">
  ...
  methods: {
    ...
    getOrderData() {
      ...
      let callback = result => {
        ...
        if (resData.has_error) {
          getApp().globalData.showError(resData.error_msg);
        }
        ...
        _this.goods_list = resData.goods_list
        _this.address = resData.address
        _this.order_total_num = resData.order_total_num
        _this.order_total_price = resData.order_total_price
        _this.coupon_list = resData.coupon_list
        _this.order_pay_price = resData.order_pay_price
      ...
      getApp().globalData._get('sharing.order/checkout', {
    ...
    onSubmitOrder() {
      ...
      const onCommitCallback = () => {
        ...
        getApp().globalData._post_form('sharing.order/checkout', {
    ...
    onVerifyFun() {
      ...
      if (_this.has_error) {
        getApp().globalData.showError(_this.error_msg);
        return false;
      } // 验证自提填写的联系方式
      ...
      if (_this.curDelivery == DeliveryTypeEnum.EXTRACT.value) {
        ...
        if (_this.selectedShopId <= 0) {
          getApp().globalData.showError('请选择自提的门店');
          return false;
        }

        if (Verify.isEmpty(_this.linkman)) {
          getApp().globalData.showError('请填写自提联系人');
          return false;
        }

        if (Verify.isEmpty(_this.phone)) {
          getApp().globalData.showError('请填写自提联系电话');
          return false;
        }

        if (!Verify.isPhone(_this.phone)) {
          getApp().globalData.showError('请输入正确的联系电话');
          return false;
        }
      }
    ...
    onSelectPayType(e) {
      ...
      getApp().globalData.saveFormId(e.detail.formId); // 设置当前支付方式
    ...
    onShowPoints(e) {
      ...
      getApp().globalData.saveFormId(e.detail.formId); // 显示dialog
